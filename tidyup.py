...
PROMPT = """
# Instruction
Analyze the input image. Detect distinct objects and classify them using the `Object List` below. 
...
def main():
    ...
    # navigate
    chassis.move_to(*POINT_DINING_ROOM)
    while not rospy.is_shutdown():
    ...
    while True:
	    ...
	    text = generate_content(PROMPT, "./image.jpg")
	    text = text.replace("\n", "")
	    text = text.replace("\r", "")
	    print("Gemini Res", text)
	
		pattern = r"```json\s(.*?)\s```"  # Corrected regex pattern
		match = re.search(pattern, text)

		if match:
		    extracted_json = match.group(1)  # Extract the content inside ```json ... ```
    ...
    for i in json_object:
    	print(i)
    	respeaker.say(f"Help me take {i["object"]}")
    	t.sleep(5)
